 ...
 * Everything in this file is always enabled.
 * It does not create an application shell, router, player or replacement pages.
 */
(function () {
    "use strict";
        ...
        for (const section of findContinueWatchingSections()) {
            ...
            for (const card of cards) {
                ...
                if (!groups.has(item.SeriesId)) {
                    groups.set(item.SeriesId, { card, episode: item });
                } else {
                    hideDuplicate(card);
                }
            }
            ...
            for (const { card, episode } of groups.values()) {
                ...
                const runtime = episode.RunTimeTicks
                    ? Math.max(1, Math.round(episode.RunTimeTicks / 600000000))
                    : 0;
                ...
                const meta = [
                    Number.isFinite(episode.ParentIndexNumber) && Number.isFinite(episode.IndexNumber)
                        ? `S${String(episode.ParentIndexNumber).padStart(2, "0")} · E${String(episode.IndexNumber).padStart(2, "0")}`
                        : "",
                    minutes
                ].filter(Boolean).join(" · ");
    ...
    function renderTechnical(host, item) {
        ...
        if (video) {
            const quality = video.Width >= 3800
                ? "4K"
                : video.Width >= 1900
                    ? "1080p"
                    : video.Width
            ...
            parts.push(
                `VIDEO ${quality}`
                + (video.VideoRange ? ` · ${video.VideoRange}` : "")
        ...
        if (audio) {
            parts.push(
                `AUDIO ${audio.Codec ? audio.Codec.toUpperCase() : "AUDIO"}`
        ...
        if (subs.length) {
            const languages = [...new Set(subs.map(s => s.Language || s.Codec).filter(Boolean))];
            parts.push(`SUBTITLES ${languages.join(", ") || subs.length}`);
        }
    ...
    async function enhanceDetails() {
        ...
        renderTechnical(
            page.querySelector(".overview, .infoWrapper, .itemMiscInfo") || host,
            item
        );
    }
