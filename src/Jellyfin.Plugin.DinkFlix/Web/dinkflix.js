/* jshint esversion: 11, browser: true, devel: true, asi: false, lastsemic: true, undef: false, unused: false */
/* global fetch */
/* DINKFLIX Web 4.1 — Jellyfin 12.1 desktop frontend. */
(() => {
    'use strict';
    if (window.__DINKFLIX_WEB_41__)
        return;
    window.__DINKFLIX_WEB_41__ = true;
    const VERSION = '4.1.0';
    const state = {
    ...
    const humanMinutes = (ticks) => { const n = Number(ticks || 0); if (!n)
        return ''; const m = Math.round(n / 600000000); if (m < 60)
        return `${m} min`; const h = Math.floor(m / 60), r = m % 60; return r ? `${h}h ${r}m` : `${h}h`; };
    const fmtDate = (iso) => { try {
        return new Intl.DateTimeFormat(undefined, { year: 'numeric' }).format(new Date(iso));
    }
    catch {
        return '';
    } };
    ...
    function credentials() {
        try {
            ...
            const match = servers.find(s => ['ManualAddress', 'RemoteAddress', 'LocalAddress'].some(k => { try {
                return s?.[k] && new URL(s[k]).origin === origin;
            }
            catch {
                return false;
            } }));
    ...
    function baseUrl() {
        ...
        const i = p.toLowerCase().indexOf('/web/');
        if (i >= 0)
            return location.origin + p.slice(0, i);
        if (p.toLowerCase().endsWith('/web'))
            return location.origin + p.slice(0, -4);
    ...
    function image(item, type = 'Primary', maxWidth = 720) {
        ...
        const p = new URLSearchParams({ maxWidth: String(maxWidth), quality: '86' });
        if (tag)
            p.set('tag', tag);
        const t = token();
        if (t)
    ...
    function backdrop(item, maxWidth = 1800) {
        if (!item?.Id)
            return '';
        const type = ['Backdrop', 'Primary', 'Thumb'].find(x => item?.ImageTags?.[x]);
        if (!type)
            return '';
        const tag = item?.ImageTags?.[type];
        const p = new URLSearchParams({ maxWidth: String(maxWidth), quality: '84' });
        if (tag)
            p.set('tag', tag);
        const t = token();
        if (t)
    ...
    function route() {
        ...
        if (path === '/home' && df === 'library')
            return { type: 'library', viewId: params.get('viewId') };
        if (path === '/home' && df === 'item')
            return { type: 'item', id: params.get('id') };
        if (path === '/home' && df === 'list')
            return { type: 'list' };
        if (path === '/home' && df === 'about')
            return { type: 'about' };
        if (path === '/home' && df === 'search')
            return { type: 'search', q: params.get('q') || '' };
        if (path === '/home' && !params.get('tab'))
            return { type: 'home' };
        if (path === '/search')
            return { type: 'search', q: params.get('query') || params.get('q') || '' };
        ...
        if (path === '/details' && params.get('id') && !params.get('dfnative'))
            return { type: 'item', id: params.get('id') };
        if (path === '/video' || path === '/playback' || path === '/nowplaying' || path === '/fullscreen')
            return { type: 'playback' };
        if (path === '/home' && params.get('tab'))
            return { type: 'native', reason: 'home-tab' };
    ...
    async function loadUser() {
        ...
        if (!state.userId)
            return;
        try {
            state.user = await api(`/Users/${encodeURIComponent(state.userId)}`);
        }
        catch { }
    }
    async function loadViews() {
        if (!state.userId)
            return;
        try {
            state.views = await api(`/Users/${encodeURIComponent(state.userId)}/Views`) || [];
        }
        catch {
            state.views = [];
        }
    ...
    async function loadConfig() {
        try {
            const r = await api('/Plugins/DinkFlixWeb/Client/Configuration');
            state.config = { ...state.config, ...r };
            if (/^#[\da-f]{6}$/i.test(state.config.accentColor))
    ...
    function viewLabel(v) {
        ...
        if (c === 'tvshows')
            return 'TV Shows';
        return v.Name || 'Library';
    }
    function viewHref(v) {
        const c = String(v.CollectionType || '').toLowerCase();
        if (c === 'movies')
            return `#/movies?topParentId=${encodeURIComponent(v.Id)}&collectionType=movies`;
        if (c === 'tvshows')
            return `#/tv?topParentId=${encodeURIComponent(v.Id)}&collectionType=tvshows`;
        return `#/home?df=library&viewId=${encodeURIComponent(v.Id)}`;
    }
    function svg(name) {
        const m = {
    ...
    function makeNav() {
        ...
        nav.innerHTML = `<div class="df-nav-inner"><a href="#/home" class="df-brand" data-df-action="home" aria-label="DINKFLIX home"><span>DINK</span><span>FLIX</span></a><div class="df-nav-links" id="df-nav-links"></div><div class="df-nav-spacer"></div><div class="df-nav-tools"><button class="df-icon-btn" id="df-search-btn" aria-label="Search" title="Search">${svg('search')}</button><button class="df-icon-btn" id="df-tools-btn" aria-label="Tools" title="Tools">${svg('tools')}</button><button class="df-icon-btn" id="df-user-btn" aria-label="User menu" title="User menu"><span class="df-avatar" id="df-avatar">?</span></button></div></div>`;
    ...
    function updateNav() {
        ...
        if (state.config.showRequests !== false)
            html += `<a class="df-nav-link df-native-nav ${r.type === 'native' && r.reason === 'home-tab' && parseHash().params.get('tab') === '2' ? 'df-active' : ''}" href="#/home?tab=2">Requests</a>`;
        if (state.config.showBookmarks !== false)
            html += `<a class="df-nav-link df-native-nav ${r.type === 'native' && r.reason === 'home-tab' && parseHash().params.get('tab') === '3' ? 'df-active' : ''}" href="#/home?tab=3">Bookmarks</a>`;
    ...
    function navigateToView(v) {
        const c = String(v.CollectionType || '').toLowerCase();
        if (c === 'movies')
            nativeHash('/movies', { topParentId: v.Id, collectionType: 'movies' });
        else if (c === 'tvshows')
            nativeHash('/tv', { topParentId: v.Id, collectionType: 'tvshows' });
        else
            setHash('/home', { df: 'library', viewId: v.Id });
    }
    function discoverNativeLinks() {
        const anchors = [...document.querySelectorAll('a[href]')];
        let requests = false, bookmarks = false;
        anchors.forEach(a => { const h = a.getAttribute('href') || ''; if (/tab=2/.test(h) || /request/i.test(a.textContent || ''))
            requests = true; if (/tab=3/.test(h) || /bookmark/i.test(a.textContent || ''))
            bookmarks = true; });
        return { requests, bookmarks };
    }
    function avatar() {
        const el = document.getElementById('df-avatar');
        if (!el)
            return;
        const tag = state.user?.PrimaryImageTag;
        if (tag) {
            const p = new URLSearchParams({ tag, maxWidth: '72' });
            const t = token();
            if (t)
                p.set('ApiKey', t);
            el.innerHTML = `<img alt="" src="${esc(`${baseUrl()}/Users/${encodeURIComponent(state.userId)}/Images/Primary?${p}`)}">`;
        }
        else
            el.textContent = (String(state.user?.Name || '?').trim()[0] || '?').toUpperCase();
    }
    function scrollNav() { const n = state.nav; if (!n)
        return; n.classList.toggle('df-scrolled', window.scrollY > 18); }
    function ensureShell() { if (state.shell)
        return state.shell; const s = document.createElement('main'); s.id = 'dinkflix-app-shell'; document.body.appendChild(s); state.shell = s; return s; }
    function ensureMenuRoot() { if (state.menuRoot)
        return state.menuRoot; const d = document.createElement('div'); d.id = 'dinkflix-menu-root'; document.body.appendChild(d); state.menuRoot = d; return d; }
    function ensureToastRoot() { if (state.toastRoot)
        return state.toastRoot; const d = document.createElement('div'); d.id = 'dinkflix-toasts'; document.body.appendChild(d); state.toastRoot = d; return d; }
    ...
    function positionMenu(menu, anchor, align = 'right') {
        ...
        if (top + mh > innerHeight - 10)
            top = Math.max(10, rect.top - mh - 7);
    ...
    function openLibrariesMenu(anchor, views) {
        ...
        m.className = 'df-more-menu';
        views.forEach(v => m.append(menuButton(viewLabel(v), 'movie', () => navigateToView(v))));
        positionMenu(m, anchor);
        ensureMenuRoot().appendChild(m);
    }
    function openToolsMenu(anchor) {
        closeMenus();
        const m = document.createElement('div');
        m.className = 'df-user-menu';
        m.append(menuButton('Surprise me', 'movie', randomTitle));
    ...
    function openUserMenu(anchor) {
        ...
        if (isAdmin())
            m.append(menuButton('Dashboard', 'tools', () => nativeHash('/dashboard')));
    ...
    function nativeAction(regex) {
        const candidates = [...document.querySelectorAll('button,a,[role="button"]')].filter(e => !e.closest('#dinkflix-nav,#dinkflix-menu-root'));
        const found = candidates.find(e => regex.test(`${e.textContent || ''} ${e.getAttribute('aria-label') || ''} ${e.getAttribute('title') || ''}`));
        if (found) {
            found.click();
            return true;
        }
        toast('That Jellyfin action is not available here.');
        return false;
    }
    async function randomTitle() {
        try {
            const items = await getItems({ IncludeItemTypes: 'Movie,Series', Limit: 100, SortBy: 'Random' });
            if (items.length)
    ...
    function badgeHtml(item) {
        ...
        if (state.config.showRatings && rating)
            b.push(`<span class="df-badge rating">★ ${rating.toFixed(1)}</span>`);
        const w = Number(item?.Width || 0), h = Number(item?.Height || 0);
        if (state.config.showMediaBadges && (h >= 2000 || w >= 3500))
            b.push('<span class="df-badge quality">4K</span>');
        else if (state.config.showMediaBadges && (h >= 1080 || w >= 1900))
            b.push('<span class="df-badge quality">1080p</span>');
        if (state.config.showMediaBadges && (item?.VideoRange || /hdr/i.test(item?.VideoRangeType || '')))
            b.push('<span class="df-badge hdr">HDR</span>');
        if (item?.OfficialRating)
            b.push(`<span class="df-badge age">${esc(item.OfficialRating)}</span>`);
        return b.join('');
    }
    function tagHtml(item) { return (item?.Tags || []).slice(0, 2).map(t => `<span class="df-badge tag">${esc(t)}</span>`).join(''); }
    function card(item) {
        const id = item.Id, title = esc(item.Name), src = image(item, 'Primary', 720), p = percent(item);
        const playable = item.Type === 'Movie' || item.Type === 'Episode' || item.Type === 'Video';
        return `<article class="df-card" data-id="${esc(id)}" data-kind="${esc(item.Type || '')}" tabindex="-1"><a class="df-card-link" href="#/home?df=item&id=${encodeURIComponent(id)}" data-df-item="${esc(id)}" aria-label="Open ${title}"></a><div class="df-card-media"><img loading="lazy" src="${esc(src)}" alt="${title}" onerror="this.style.display='none'"><div class="df-card-shade"></div>${playable ? `<button class="df-card-play-btn" data-df-play="${esc(id)}" aria-label="Play ${title}" title="Play">${svg('play')}</button>` : ''}<button class="df-card-menu-btn" data-df-menu="${esc(id)}" aria-label="More actions for ${title}" title="More actions">${svg('dots')}</button><div class="df-card-content"><div class="df-card-title">${title}</div><div class="df-card-meta"><span>${esc(fmtDate(item.DateCreated || item.PremiereDate))}</span>${humanMinutes(item.RunTimeTicks) ? `<span>•</span><span>${humanMinutes(item.RunTimeTicks)}</span>` : ''}</div><div class="df-card-badges">${badgeHtml(item)}${tagHtml(item)}</div></div>${p > 0 ? `<div class="df-progress"><span style="width:${p}%"></span></div>` : ''}</div></article>`;
    }
    async function getItems(options = {}) {
        const defaults = { UserId: state.userId, Recursive: true, IncludeItemTypes: 'Movie,Series', Fields: 'PrimaryImageAspectRatio,Overview,Genres,Tags,People,MediaSources,ProviderIds,UserData,OfficialRating,CommunityRating,ProductionYear,RunTimeTicks,Width,Height,VideoRange,VideoRangeType,DateCreated,DatePlayed,ChildCount', Limit: 24, StartIndex: 0, SortBy: 'SortName', SortOrder: 'Ascending' };
        const p = new URLSearchParams();
        Object.entries({ ...defaults, ...options }).forEach(([k, v]) => { if (v !== undefined && v !== null && v !== '')
            p.set(k, String(v)); });
        const r = await api(`/Users/${encodeURIComponent(state.userId)}/Items?${p}`);
    ...
    function bindCardActions(root) {
        ...
        root.querySelectorAll('[data-df-play]').forEach(b => {
            if (b.dataset.dfBound)
                return;
            b.dataset.dfBound = '1';
            b.addEventListener('click', e => { e.preventDefault(); e.stopPropagation(); const item = cardData.get(b.dataset.dfPlay); if (item)
                playItem(item); });
        });
    }
    ...
    async function openPicker(title, items, onChoose) {
        ...
        list.querySelectorAll('[data-picker-id]').forEach(b => b.onclick = async () => { try {
            await onChoose(b.dataset.pickerId);
            m.remove();
        }
        catch {
    ...
    async function openCardMenu(anchor, id) {
        closeMenus();
        const item = cardData.get(id) || await getItem(id).catch(() => null);
        if (!item) {
            toast('Unable to load this title.');
            return;
        }
        const m = document.createElement('div');
        m.className = 'df-context-menu';
    ...
    async function toggleFavorite(item) {
        ...
        await api(`/Users/${encodeURIComponent(state.userId)}/FavoriteItems/${encodeURIComponent(item.Id)}`, { method: fav ? 'DELETE' : 'POST' });
        item.UserData = item.UserData || {};
        item.UserData.IsFavorite = !fav;
        cardData.set(item.Id, item);
        toast(fav ? 'Removed from My List' : 'Added to My List');
        return !fav;
    }
    function downloadItem(item) { const p = new URLSearchParams(); const t = token(); if (t)
        p.set('ApiKey', t); window.open(`${baseUrl()}/Items/${encodeURIComponent(item.Id)}/Download${p.toString() ? `?${p}` : ''}`, '_blank', 'noopener'); }
    async function copyStreamUrl(item) {
        const p = new URLSearchParams({ Static: 'true' });
        const t = token();
        if (t)
            p.set('ApiKey', t);
        const url = `${baseUrl()}/Videos/${encodeURIComponent(item.Id)}/stream?${p}`;
        try {
    ...
    async function refreshMetadata(item) { try {
        await api(`/Items/${encodeURIComponent(item.Id)}/Refresh`, { method: 'POST' });
    ...
    async function playFromHere(item) {
        if (item.Type === 'Series') {
            try {
                const eps = await getItems({ ParentId: item.Id, IncludeItemTypes: 'Episode', Limit: 100, SortBy: 'ParentIndexNumber,IndexNumber', SortOrder: 'Ascending', Fields: 'UserData,RunTimeTicks,MediaSources' });
                const next = eps.find(e => !e.UserData?.Played) || eps[0];
                if (next)
    ...
    async function getPlaybackManager() {
        ...
        if (typeof window.require === 'function')
            return new Promise(resolve => { try {
                window.require(['playbackManager'], m => resolve(m), () => resolve(null));
            }
            catch {
                resolve(null);
            } });
    ...
    async function playItem(item) {
        ...
        if (pm?.play) {
            try {
                await pm.play({ items: [item], fullscreen: true, enableRemotePlayers: true });
                return;
            }
            catch { }
        }
    ...
    function autoPlayFallback() {
        ...
        if (!id)
            return;
        let n = 0;
        const timer = setInterval(() => { n++; const els = [...document.querySelectorAll('button,a')].filter(e => !e.closest('#dinkflix-nav')); const b = els.find(e => /^(resume|play)$/i.test((e.textContent || '').trim())); if (b) {
    ...
    function ensurePlaybackOverlay() {
        ...
        el.id = 'dinkflix-playback-overlay';
    ...
    async function renderHome() {
        const s = ensureShell();
        s.innerHTML = '<div class="df-home-loading"></div>';
        try {
            const [recent, resume, played, favs] = await Promise.all([
                getItems({ IncludeItemTypes: 'Movie,Series', SortBy: 'DateCreated', SortOrder: 'Descending', Limit: 20 }),
            ...
            setCardData([...recent, ...resume, ...played, ...favs]);
            const hero = recent.filter(x => x.ImageTags?.Backdrop || x.ImageTags?.Primary).slice(0, 8);
            state.heroItems = hero;
            ...
            const movieHref = movieView ? viewHref(movieView) : '';
            const tvHref = tvView ? viewHref(tvView) : '';
            s.innerHTML = `<div class="df-hero" id="df-hero"><div class="df-hero-media" id="df-hero-media"></div><div class="df-hero-overlay"></div><div class="df-hero-content" id="df-hero-content"></div><div class="df-hero-dots" id="df-hero-dots"></div></div><div class="df-page">${section('Continue Watching', resume)} ${section('Recently Played', played)} ${section('Recently Added', recent)} ${section('Movies', recent.filter(x => x.Type === 'Movie'), movieHref)} ${section('TV Shows', recent.filter(x => x.Type === 'Series'), tvHref)}${state.config.showMyList ? section('My List', favs, '#/home?df=list') : ''}</div>`;
            bindCardActions(s);
            if (hero.length)
                showHero(0);
            else
                s.querySelector('#df-hero').remove();
        }
        catch (err) {
            s.innerHTML = `<div class="df-page"><div class="df-empty">DINKFLIX couldn't load your library. Check the browser console for the server response.</div></div>`;
            console.error('[DINKFLIX]', err);
        }
    }
    function showHero(idx) {
        ...
        const c = document.getElementById('df-hero-content');
        const dots = document.getElementById('df-hero-dots');
        if (!hero || !media || !c)
            return;
        media.style.backgroundImage = `url("${backdrop(item, 2000)}")`;
        hero.classList.add('df-ready');
        c.innerHTML = `<div class="df-hero-eyebrow">Recently added</div><h1 class="df-hero-title">${esc(item.Name)}</h1><div class="df-hero-meta"><span>${esc(fmtDate(item.PremiereDate || item.DateCreated))}</span>${humanMinutes(item.RunTimeTicks) ? `<span>•</span><span>${humanMinutes(item.RunTimeTicks)}</span>` : ''}<span>•</span><span>${esc(item.Type === 'Series' ? 'TV Series' : 'Movie')}</span>${badgeHtml(item)}</div><p class="df-hero-overview">${esc(item.Overview || '')}</p><div class="df-actions"><button class="df-button df-button-primary" id="df-hero-play">${svg('play')} Play</button><button class="df-button df-button-secondary" id="df-hero-info">More info</button><button class="df-button df-button-secondary" id="df-hero-list">${item.UserData?.IsFavorite ? '✓ In My List' : '＋ My List'}</button></div>`;
        dots.innerHTML = state.heroItems.map((_, i) => `<button class="df-hero-dot ${i === idx ? 'df-current' : ''}" data-i="${i}" aria-label="Hero ${i + 1}"></button>`).join('');
        dots.querySelectorAll('button').forEach(b => b.onclick = () => { stopHero(); showHero(Number(b.dataset.i)); startHero(); });
        c.querySelector('#df-hero-play').onclick = () => playItem(item);
        c.querySelector('#df-hero-info').onclick = () => setHash('/home', { df: 'item', id: item.Id });
        c.querySelector('#df-hero-list').onclick = async () => { await toggleFavorite(item); showHero(idx); };
        const root = hero;
        root.onmouseenter = () => state.heroPaused = true;
        root.onmouseleave = () => state.heroPaused = false;
        root.onfocusin = () => state.heroPaused = true;
        root.onfocusout = () => state.heroPaused = false;
    }
    function startHero() { stopHero(); const sec = Math.max(8, Number(state.config.heroRotationSeconds) || 14); if (state.heroItems.length > 1)
        state.heroTimer = setInterval(() => { if (!state.heroPaused && !document.hidden)
            showHero((state.heroIndex + 1) % state.heroItems.length); }, sec * 1000); }
    function stopHero() { if (state.heroTimer) {
        clearInterval(state.heroTimer);
        state.heroTimer = null;
    } }
    async function renderLibrary(kind) {
        const s = ensureShell();
        let view = state.views.find(v => v.Id === kind?.viewId);
        if (!view && kind?.type === 'library-native' && kind.viewId)
            view = state.views.find(v => v.Id === kind.viewId);
        const viewId = view?.Id || kind?.viewId;
        const title = view ? viewLabel(view) : (kind?.collectionType === 'tvshows' ? 'TV Shows' : 'Movies');
        s.innerHTML = `<div class="df-page"><header class="df-page-header"><div><div class="df-kicker">DINKFLIX library</div><h1 class="df-title">${esc(title)}</h1><p class="df-copy">Browse ${esc(title.toLowerCase())} with the DINKFLIX card system.</p></div></header><div class="df-library-toolbar"><input class="df-input df-grow" id="df-lib-search" type="search" placeholder="Search this library…"><select class="df-select" id="df-lib-sort"><option value="SortName">A–Z</option><option value="DateCreated">Recently added</option><option value="CommunityRating">Rating</option><option value="ProductionYear">Year</option></select><button class="df-chip df-selected" data-filter="all">All</button><button class="df-chip" data-filter="quality">4K</button><button class="df-chip" data-filter="hdr">HDR</button></div><div class="df-grid" id="df-lib-grid"><div class="df-empty" style="grid-column:1/-1">Loading…</div></div></div>`;
        let items = [];
        try {
            const type = String(view?.CollectionType || kind?.collectionType || '').toLowerCase();
            const include = type === 'movies' ? 'Movie' : type === 'tvshows' ? 'Series' : 'Movie,Series,Video,Audio,Book';
            items = await getItems({ ParentId: viewId, IncludeItemTypes: include, Limit: 120, SortBy: 'SortName', SortOrder: 'Ascending' });
        }
        catch { }
        setCardData(items);
        renderLibraryGrid(items, s);
        s.querySelector('#df-lib-search').oninput = () => renderLibraryGrid(items, s);
        s.querySelector('#df-lib-sort').onchange = () => renderLibraryGrid(items, s);
        s.querySelectorAll('[data-filter]').forEach(b => b.onclick = () => { s.querySelectorAll('[data-filter]').forEach(x => x.classList.remove('df-selected')); b.classList.add('df-selected'); b.dataset.active = '1'; renderLibraryGrid(items, s, b.dataset.filter); });
        bindCardActions(s);
    }
    function renderLibraryGrid(all, s, filter = 'all') {
        const q = (s.querySelector('#df-lib-search')?.value || '').trim().toLowerCase();
        const sort = s.querySelector('#df-lib-sort')?.value || 'SortName';
        let a = all.filter(i => !q || [i.Name, ...(i.Genres || []), ...(i.Tags || [])].some(x => String(x).toLowerCase().includes(q)));
        if (filter === 'quality')
            a = a.filter(i => Number(i.Height) >= 2000 || Number(i.Width) >= 3500);
        if (filter === 'hdr')
            a = a.filter(i => i.VideoRange || /hdr/i.test(i.VideoRangeType || ''));
        a.sort((x, y) => { if (sort === 'CommunityRating')
            return Number(y.CommunityRating || 0) - Number(x.CommunityRating || 0); if (sort === 'ProductionYear')
            return Number(y.ProductionYear || 0) - Number(x.ProductionYear || 0); if (sort === 'DateCreated')
            return String(y.DateCreated || '').localeCompare(String(x.DateCreated || '')); return String(x.SortName || x.Name).localeCompare(String(y.SortName || y.Name)); });
        const grid = s.querySelector('#df-lib-grid');
        if (grid)
            grid.innerHTML = a.length ? a.map(card).join('') : '<div class="df-empty" style="grid-column:1/-1">Nothing matched that filter.</div>';
        if (grid)
            bindCardActions(grid);
    }
    async function renderItem(id) {
        const s = ensureShell();
        s.innerHTML = '<div class="df-page"><div class="df-empty">Loading title…</div></div>';
        let item;
        try {
        ...
        if (!item) {
            s.innerHTML = '<div class="df-page"><div class="df-empty">This title could not be loaded.</div></div>';
            return;
        }
        setCardData([item]);
        const cast = (item.People || []).filter(p => p?.PersonId).slice(0, 10);
        const bg = backdrop(item, 2200), poster = image(item, 'Primary', 720);
        s.innerHTML = `<article class="df-detail"><div class="df-detail-bg" style="background-image:url('${esc(bg)}')"></div><div class="df-detail-vignette"></div><div class="df-detail-content"><div class="df-detail-poster"><img src="${esc(poster)}" alt="${esc(item.Name)}"></div><div><div class="df-kicker">${esc(item.Type === 'Series' ? 'TV Series' : 'Movie')}</div><h1 class="df-detail-title">${esc(item.Name)}</h1><div class="df-detail-meta"><span>${esc(fmtDate(item.PremiereDate || item.DateCreated))}</span>${humanMinutes(item.RunTimeTicks) ? `<span>•</span><span>${humanMinutes(item.RunTimeTicks)}</span>` : ''}${badgeHtml(item)}</div><div class="df-detail-tags">${(item.Genres || []).slice(0, 4).map(g => `<span class="df-badge tag">${esc(g)}</span>`).join('')}${tagHtml(item)}</div><p class="df-detail-overview">${esc(item.Overview || 'No overview available.')}</p><div class="df-actions"><button class="df-button df-button-primary" id="df-detail-play">${svg('play')} ${item.UserData?.PlaybackPositionTicks ? 'Resume' : 'Play'}</button><button class="df-button df-button-secondary" id="df-detail-list">${item.UserData?.IsFavorite ? '✓ In My List' : '＋ My List'}</button><button class="df-button df-button-secondary" id="df-detail-more">${svg('dots')} More</button></div>${cast.length ? `<div class="df-detail-cast"><h3>Cast</h3><div class="df-cast-row">${cast.map(p => `<div class="df-cast"><img loading="lazy" src="${esc(p.PrimaryImageTag ? `${baseUrl()}/Persons/${encodeURIComponent(p.PersonId)}/Images/Primary?tag=${encodeURIComponent(p.PrimaryImageTag)}&maxWidth=168` : '')}" alt="${esc(p.Name || '')}"><div class="df-cast-name">${esc(p.Name || '')}</div></div>`).join('')}</div></div>` : ''}</div></div></article>`;
        s.querySelector('#df-detail-play').onclick = () => playItem(item);
        s.querySelector('#df-detail-list').onclick = () => toggleFavorite(item).then(async () => { const n = await getItem(item.Id); renderItem(n.Id); });
        s.querySelector('#df-detail-more').onclick = (e) => openCardMenu(e.currentTarget, item.Id);
    }
    async function renderList() {
        const s = ensureShell();
        s.innerHTML = `<div class="df-page"><header class="df-page-header"><div><div class="df-kicker">Your saved titles</div><h1 class="df-title">My List</h1><p class="df-copy">Things you saved to watch later.</p></div></header><div class="df-grid" id="df-list-grid"><div class="df-empty" style="grid-column:1/-1">Loading…</div></div></div>`;
        const items = await getItems({ IncludeItemTypes: 'Movie,Series', IsFavorite: true, SortBy: 'DatePlayed,SortName', SortOrder: 'Descending', Limit: 120 }).catch(() => []);
        setCardData(items);
        s.querySelector('#df-list-grid').innerHTML = items.length ? items.map(card).join('') : '<div class="df-empty" style="grid-column:1/-1">Your list is empty.</div>';
        bindCardActions(s);
    }
    async function renderSearch(q) {
        const s = ensureShell();
        s.innerHTML = `<div class="df-page df-search"><header class="df-page-header"><div><div class="df-kicker">DINKFLIX search</div><h1 class="df-title">Find something</h1></div></header><form class="df-search-bar" id="df-search-form"><input class="df-input df-grow" id="df-search-input" value="${esc(q)}" placeholder="Title, genre or tag…" autofocus><button class="df-button df-button-primary" type="submit">Search</button></form><p id="df-search-note" class="df-copy"></p><div class="df-grid" id="df-search-grid" style="margin-top:24px"></div></div>`;
        const form = s.querySelector('#df-search-form'), input = s.querySelector('#df-search-input');
        const run = async () => { const term = input.value.trim(); if (!term) {
            return;
        } const items = await getItems({ SearchTerm: term, IncludeItemTypes: 'Movie,Series', Limit: 120, SortBy: 'SortName' }).catch(() => []); setCardData(items); s.querySelector('#df-search-note').textContent = `${items.length} result${items.length === 1 ? '' : 's'} for “${term}”`; s.querySelector('#df-search-grid').innerHTML = items.length ? items.map(card).join('') : '<div class="df-empty" style="grid-column:1/-1">No titles matched that search.</div>'; bindCardActions(s); };
        form.onsubmit = e => { e.preventDefault(); setHash('/search', { q: input.value.trim() }); };
        if (q)
            await run();
    }
    function renderNativeAbout() {
        const s = ensureShell();
        s.innerHTML = `<div class="df-page"><header class="df-page-header"><div><div class="df-kicker">The project</div><h1 class="df-title">About DINKFLIX</h1><p class="df-copy">A personal Jellyfin experience built for the people who use the server: friends, family and anyone who wants to sit down, find something good and press play.</p></div></header><section class="df-about-grid"><div class="df-empty" style="min-height:360px;display:grid;place-items:center">DINKFLIX image slot<br><small>Replace assets/about/ placeholders in the repository.</small></div><div><h2>Why it exists</h2><p class="df-copy">DINKFLIX takes the power of Jellyfin and gives the web client a calmer, more considered interface. The aim is simple: less software-looking chrome, better discovery, clear actions and a visual identity that feels like a product rather than a skin.</p><p class="df-copy">Fast over flashy. Clear over clever. Familiar enough for guests, distinctive enough to feel like DINKFLIX.</p></div></section></div>`;
    }
    function handleRoute() {
        ...
        if (alreadyRendered && !['native', 'playback'].includes(r.type))
            return;
        state.renderKey = key;
        state.route = r;
        setBody(r);
        ...
        else if (r.type === 'playback') {
            autoPlayFallback();
        }
    }
    function bindGlobal() {
        if (!state.navScrollBound) {
            window.addEventListener('scroll', scrollNav, { passive: true });
            document.addEventListener('click', e => { if (!e.target.closest('#dinkflix-menu-root,#dinkflix-nav'))
                closeMenus(); });
            state.navScrollBound = true;
        }
        if (!state.clickBound) {
            window.addEventListener('hashchange', () => setTimeout(handleRoute, 25));
            window.addEventListener('popstate', () => setTimeout(handleRoute, 25));
    ...
    function migrateLegacyQuery() {
        ...
        if (!isHomeHash)
            return;
        if (legacy === 'movies') {
            const v = visibleViews().find(x => String(x.CollectionType || '').toLowerCase() === 'movies');
            if (v)
                navigateToView(v);
        }
        else if (legacy === 'shows') {
            const v = visibleViews().find(x => String(x.CollectionType || '').toLowerCase() === 'tvshows');
            if (v)
                navigateToView(v);
        }
        else if (legacy === 'list')
    ...
    async function boot() {
        ...
        await Promise.all([loadViews(), loadConfig()]);
        migrateLegacyQuery();
        makeNav();
        bindGlobal();
        handleRoute();
