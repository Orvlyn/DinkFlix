/* DINKFLIX Web 2.1 — lightweight desktop-first Jellyfin experience. */
(() => {
  'use strict';

  if (window.__DINKFLIX_WEB_21__) return;
  window.__DINKFLIX_WEB_21__ = true;
  ...
  const getUserId = () => {
    try { return api()?.getCurrentUserId?.() || null; } catch { return null; }
  };

  const getBaseUrl = () => {
    try { return api()?.serverAddress?.() || window.location.origin; } catch { return window.location.origin; }
  };

  const loadConfig = async () => {
    try {
      const response = await fetch(`${getBaseUrl()}/Plugins/DinkFlixWeb/Client/Configuration`, { credentials: 'include', cache: 'no-store' });
  ...
  const getItems = async (options = {}) => {
    ...
    try {
      const result = await api().getItems(state.userId, { ...defaults, ...options });
  ...
  const getItem = async (id) => {
    ...
    try {
      return await api().getItem(state.userId, id);
    } catch (error) {
  ...
  const openNativeDetails = (id, autoPlay = false) => {
    ...
    window.location.hash = `#/details?id=${encodeURIComponent(id)}&serverId=${encodeURIComponent(api()?.serverId?.() || '')}`;
  };

  const tryNativePlayback = async (item) => {
    const manager = window.playbackManager || window.PlaybackManager;
    if (!manager || typeof manager.play !== 'function') return false;
    try {
      const position = Number(item.UserData?.PlaybackPositionTicks || 0);
      await manager.play({
        ids: [item.Id],
        serverId: api()?.serverId?.(),
        startPositionTicks: position > 0 ? position : undefined
      });
      return true;
    } catch (error) {
      console.debug('[DINKFLIX] Native playback manager was unavailable:', error);
      return false;
    }
  };
  ...
  const makeNav = () => {
    ...
    nav.innerHTML = `
      <div class="df-nav-inner">
        ...
        <div class="df-nav-links">
          <a class="df-nav-link" data-route="home" href="?df=home" data-df-route="home"><span class="df-nav-icon" aria-hidden="true">⌂</span><span class="df-nav-label">Home</span></a>
          <a class="df-nav-link" data-route="movies" href="?df=movies" data-df-route="movies"><span class="df-nav-icon" aria-hidden="true">▣</span><span class="df-nav-label">Movies</span></a>
          <a class="df-nav-link" data-route="shows" href="?df=shows" data-df-route="shows"><span class="df-nav-icon" aria-hidden="true">▤</span><span class="df-nav-label">TV Shows</span></a>
          <a class="df-nav-link df-mylist-nav" data-route="list" href="?df=list" data-df-route="list"><span class="df-nav-icon" aria-hidden="true">＋</span><span class="df-nav-label">My List</span></a>
          <button class="df-nav-link" id="df-requests-link" type="button" hidden><span class="df-nav-icon" aria-hidden="true">↗</span><span class="df-nav-label">Requests</span></button>
        </div>
        <div class="df-nav-spacer"></div>
        <button class="df-search-button" id="df-search" type="button" aria-label="Search" title="Search">⌕</button>
    ...
    anchorRoutes.forEach((anchor) => anchor.addEventListener('click', () => setTimeout(() => setActiveNav(routeFromHash()), 0)));
  ...
  const renderHome = async () => {
    ...
    shell.innerHTML = `
      <div class="df-page">
        ...
        ${section('Movies', data.movies, { moreHref: '#/dinkflix-movies' })}
        ${section('TV Shows', data.shows, { moreHref: '#/dinkflix-shows' })}
  ...
  const toggleMyList = async (item) => {
    if (!item || !api() || !state.userId) return false;
    const current = Boolean(item.UserData?.IsFavorite) || state.list.some((entry) => entry.Id === item.Id);
    try {
      if (current) await api().updateFavoriteStatus(state.userId, item.Id, false);
      else await api().updateFavoriteStatus(state.userId, item.Id, true);
  ...
  const boot = async () => {
    ...
    try { state.user = await api().getCurrentUser?.(); } catch { state.user = null; }
