...
/// <summary>
/// Registers and unregisters the DINKFLIX Web File Transformation with Jellyfin.
/// </summary>
public sealed class FileTransformationRegistrationService : IHostedService
{
    ...
    private async Task RegisterWithRetryAsync(CancellationToken cancellationToken)
    {
        ...
        _logger.LogWarning("DINKFLIX Web could not register its File Transformation hook. The plugin will remain loaded, but the frontend may not be injected until the server is restarted.");
    }
