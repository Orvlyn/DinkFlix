...
for css_path in (ROOT / 'dinkflix.css', ROOT / 'src/Jellyfin.Plugin.DinkFlix/Web/dinkflix.css'):
    css = css_path.read_text(encoding='utf-8')
    for token in ['--df-accent', 'html.dinkflix-native', '.card', '.itemDetailPage', '.videoOsdPage']:
        assert token in css, f'{token}: {css_path}'
    balanced(css_path)

for js_path in (ROOT / 'dinkflix.js', ROOT / 'src/Jellyfin.Plugin.DinkFlix/Web/dinkflix.js'):
    js = js_path.read_text(encoding='utf-8')
    assert '__DINKFLIX_NATIVE_THEME_V1__' in js
