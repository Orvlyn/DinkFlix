/* DINKFLIX Web 3.0 — lightweight desktop-first Jellyfin experience. */
(() => {
  'use strict';

  if (window.__DINKFLIX_WEB_30__) return;
  window.__DINKFLIX_WEB_30__ = true;
  ...
  const getAccessToken = () => {
    ...
    try { return legacyApi()?.accessToken?.() || ''; } catch { return ''; }
  };
  ...
  const getUserId = () => {
    ...
    try { return legacyApi()?.getCurrentUserId?.() || null; } catch { return null; }
  };

  const getServerId = () => {
    const stored = credentials();
    if (stored?.Id) return stored.Id;
    try { return legacyApi()?.serverId?.() || ''; } catch { return ''; }
  };
  ...
  const authHeader = () => {
    ...
    if (!token) return {};
    return {
      Authorization: `MediaBrowser Client="DINKFLIX Web", Device="Browser", DeviceId="dinkflix-web", Version="3.0.0", Token="${token}"`
    };
  };
  ...
  const openNativeDetails = (id, autoPlay = false) => {
    ...
    url.hash = `#/details?id=${encodeURIComponent(id)}&serverId=${encodeURIComponent(getServerId())}`;
  ...
  const syncBody = (route) => {
    ...
    document.body.classList.toggle('df-native-route', !isCustomRoute(route));
