...
(() => {
    ...
    async function mountHome() {
        ...
        const [heroItems, continueItems, latestMovies, latestShows, favorites] = await Promise.all([
            getRecentlyAdded(),
        ...
        root.appendChild(createRow('Recently Added', 'Fresh from the library.', [...latestMovies, ...latestShows].slice(0, CONFIG.rowLimit)));
        ...
        if (favorites.length) root.appendChild(createRow('My List', 'Titles you saved for later.', favorites, { kind: 'list', viewAllHref: '#/dinkflix-list' }));
    ...
    async function reconcile() {
        ...
        if (current === 'home' || current === 'jellyfin') {
            const page = document.querySelector(SELECTORS.home);
            if (page && visible(page)) {
                await mountHome();
